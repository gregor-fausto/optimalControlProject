...
other=initsList[[2]]
mParms=initsList[[3]]
seasonParms=initsList[[5]]
mu=seasonParms["mu"];
sigma=seasonParms["sigma"]
...
initsName <- paste0(initsList[["seasonParms"]]["sigma"],"-",initsList[["mParms"]]["m1"],"-",initsList[["mParms"]]["alpha"],gParm)
...
summaryObject <- list(fit=fit,fvals=fvals,
                      ...
                      season = if(exists("mu")) c(mu,sigma) else c(NA),
                      topt = topt,
                      iterations=length(fvals),
                      model=controlName ,
                      initsName = initsName )

saveRDS(summaryObject,paste0("~/Dropbox/optimalControlProject/discretize-optimize/analysisFour/",controlName,"-",initsName,".RDS"))
