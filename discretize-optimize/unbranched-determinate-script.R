...
pwt=10; lambda=lambda1=1; fvals = numeric(40);  
...
# f3 = approxfun(topt,tMat[,3],rule=2);
# 
# out[[1]] = data.frame(ode(y=y0,times=topt,func=control,parms=mParms,f1=f1,f2=f2,f3=f3));
# functions[[1]]<-list(f1,f2,f3)

interpolate = function(x){ return( approx(x, n = length(x)*2 - 1)$y )}
par.new=interpolate(fit$par[1:length(topt)])
par.new=c(par.new,interpolate(fit$par[(length(topt)+1):(2*length(topt))]))
par.new=c(par.new,interpolate(fit$par[(2*length(topt)+1):(3*length(topt))]))

topt=interpolate(topt)
fit$par = par.new
...
# f3 = approxfun(topt,tMat[,3],rule=2);
# 
# out[[2]] = data.frame(ode(y=y0,times=topt,func=control,parms=mParms,f1=f1,f2=f2,f3=f3));
# functions[[2]]<-list(f1,f2,f3)
...
# f3 = approxfun(topt,tMat[,3],rule=2);
# 
# out[[3]] = data.frame(ode(y=y0,times=topt,func=control,parms=mParms,f1=f1,f2=f2,f3=f3));
# functions[[3]]<-list(f1,f2,f3)
...
# out[[4]] = data.frame(ode(y=y0,times=topt,func=control,parms=mParms,f1=f1,f2=f2,f3=f3));
# functions[[4]]<-list(f1,f2,f3)
...
saveRDS(out,paste0("~/Dropbox/optimalControlProject/discretize-optimize/output/",dataName,".RDS") )
functions<-list(f1,f2,f3)
saveRDS(functions,paste0("~/Dropbox/optimalControlProject/discretize-optimize/output/",functionName,".RDS") )
