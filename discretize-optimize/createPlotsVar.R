...
a=sub(".RDS", "", unbranchedFiles)
#b=sub("m2", "", a)
c=strsplit(a, '-')
sigma=as.numeric(sapply(c, `[`, 4))

par(mfrow=c(1,1))
plot(seq(0,10,length=100),seq(0,10,length=100),type='n',
     xlab="beta_1",ylab="Fitness at t=5")
for(i in 1:n){
  points(sigma[i],unbranchedDF[[i]][11,]$L,col=colors[i],pch=16)
  points(sigma[i],max(branchedDF[[i]]$L),col=colors[i],pch=1)
}
...
par(mfrow=c(2,3))
for(i in 1:n){
  plot(seq(0,5,length=100),seq(0,2,length=100),type='n',ylab="beta2(t)")
