...
write.csv(df,"~/Dropbox/optimalControlProject/discretize-optimize/analysisSeven/parameterTable.csv",row.names=F)
#write.csv(df[-c(1:(dim(df)[1])),],"~/Dropbox/optimalControlProject/discretize-optimize/analysisSeven/exitTable.csv",row.names=F)
...
for(i in 1:dim(df)[1]){
  a1=read.csv("~/Dropbox/optimalControlProject/discretize-optimize/analysisSeven/parameterTable.csv")
  a2=read.csv("~/Dropbox/optimalControlProject/discretize-optimize/analysisSeven/exitTable.csv")
  a2$model = as.factor(a2$model)
  a2$seasonDistribution = as.factor(a2$seasonDistribution)
  subDF = setdiff(a1,a2)
  tmp=subDF[1,]
  write.table( tmp,  
               file="~/Dropbox/optimalControlProject/discretize-optimize/analysisSeven/exitTable.csv", 
               append = T, 
               sep=',', 
               row.names=F, 
               col.names=F )
