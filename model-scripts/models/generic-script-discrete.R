...
par.u = runif(length(topt),0.01,.99)
par.beta1 = ifelse(runif(length(topt),0,1)>.5,1,0)
# beta2 is maxed-out at the end in the unconstrained problem (from analysis of adjoint equations)   
# so start with the max first
par.beta2 = ifelse(runif(length(topt),0,1)>.5,1,0)

par0 = c(par.u,par.beta1,par.beta2)
