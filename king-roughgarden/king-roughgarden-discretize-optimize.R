...
source("discretize-then-optimize.R"); 
...
fit <- optim(par=uVals_init, fn=penalized_fun, method="BFGS",
...
for(j in 0:1000) {  
    theta=fit$par + (j/1000)*(uVals_init - fit$par) 
    z = min(Amat %*% theta - bvec)
    if(z>=0) break
}    

plot(seq(0,Tf,length.out=nt),theta,type='o',lty='solid',ylim=c(0,1),pch=4)
###########################################################
...
#points(seq(0,Tf,length.out=nt2),approx(fit$par,n=nt2)$y)


## Block 1
## Constraint: u-1 <= 0
## u4 positive
A1 = matrix(0,nrow=nt2,ncol=nt2);
diag(A1) = -1; c1 = rep(-1,nt2);

## Block 2
## Constraint: u >= 0
A2 = matrix(0,nrow=nt2,ncol=nt2);
diag(A2) = 1; c2 = rep(0,nt2);

# create matrix A and vector b
Amat = rbind(A1,A2);
bvec = c(c1,c2);

timesA=seq(0,Tf,length=nt2);

fit$par=approx(fit$par,n=nt2,method='linear')$y
uVals = fit$par
#points(seq(0,Tf,length.out=nt2),fit$par,pch=4)
...
# getting warning message that
# In ui.theta - ci :
# longer object length is not a multiple of shorter object length

lines(seq(0,Tf,length.out=nt2),fit$par,lty="dotted")
...
lines(seq(0,Tf,length.out=nt2),fit$par,lty="dotdash")
...
par(mfrow=c(2,1))
plot(df,uVals,type='n')
text(df,uVals,label=1:length(df))

plot(seq(0,Tf,length.out=nt2),df,type='l')
text(seq(0,Tf,length.out=nt2),df)
...
repToVeg <- out[, 3] / out[, 2]
vegToRep <- out[, 2] / out[, 3]
tstar<-c()
tstar2<-c()
