...
#  ut = max(u,0) ; bad = abs(u-ut)^1.25; 
 ...
 # if (beta1t*P + beta2t*I <= alpha*V) {
  ...
  derivs[1] = Ve - (beta1t) * ut2 * Ve
  derivs[2] = Le+(beta1t) * ( Ve )
  derivs[3] = In + beta1t * ut2 * Ve - (beta2t) * ( In )
  derivs[4] = (beta2t) * In
  derivs[5] = bad
  ...
  Fl.val = ifelse(Fl==0,0,Fl)
   val = ifelse(t>=seasonEnd-seasonEnd/2,log(Fl),0); # SPE: season's end is Uniform(2,5). 
   derivs[6] = ifelse(Fl.val==0,0,val); # SPE: season's end is Uniform(2,5). 
