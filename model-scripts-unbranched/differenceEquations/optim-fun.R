optim_fun = function(theta){
  
  tMat = matrix(theta[2:length(theta)],ncol=2); 
  
  t_switch = theta[1]
  f2 = approxfun(topt,rep(1,length(tMat[,1])),rule=2); 
  f3 = approxfun(topt,rep(1,length(tMat[,1])),rule=2); 
  y0 = c(inits,other) 
  
  out = ode(y=y0,times=0:60,control,method="iteration",parms=mParms,t_switch=t_switch,f2=f2,f3=f3);
  
  pen = out[nrow(out),"pen"]; # integrated constraint violation penalty 
  obj = out[nrow(out),"obj"]; 
  
  #wiggly = sum(diff(diff(tMat[,1])), diff(diff(tMat[,2])))#, diff(diff(tMat[,3]))); 

  val = obj - pwt*pen# - lambda*sum(wiggly^2)  ## SPE: sum instead of mean on wiggliness
  
  return(-val)
}
