...
backward = function(t,y,parms,f1,f2, s1, s2, s3 ) {
  ...
  derivs[1] = beta1t * ut * ( lambda3 - lambda1 - lambda2 ) + beta1t * (lambda1 - lambda3)
  derivs[2] = 0
  derivs[3] = -(1/L)
...
while(stop_criteria > 1e-5){
  ...
  out = ode(c(0,0,0),rev(tVals),backward,parms=parmsA ,atol = 1e-7,  f1 = uFunc, f2 = beta1Func, s1=P_Func, s2=V_Func, s3=L_Func, method="euler");
  new_lambdaVals = list(rev(out[,2]),rev(out[,3]), rev(out[,4]) );
  
  # update control using gradient, first p then q
  switch_u = (new_lambdaVals[[1]]+new_lambdaVals[[2]] - new_lambdaVals[[3]])*beta1Func(tVals)*P_Func(tVals);
  ...
  switch_b1 = (new_lambdaVals[[1]]+new_lambdaVals[[2]] - new_lambdaVals[[3]])*uFunc(tVals)*P_Func(tVals) + (new_lambdaVals[[3]] - new_lambdaVals[[1]])*P_Func(tVals);
