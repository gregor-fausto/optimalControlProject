...
backward = function(t,y,parms,f1,f2, s1, s2,s3,s4) {
  ...
  derivs[1] = -((V*lambda1-V*lambda3)*pt+V*lambda3+V*lambda2-V*lambda1)*qt
  derivs[2] = -((P*lambda1-P*lambda3)*pt-I*lambda4+P*lambda3+P*lambda2-P*lambda1)*qt-I*lambda4
...
while(stop_criteria > 1e-5){
  ...
  switch_p = (new_xVals[,1]*new_xVals[,2])*(new_lambdaVals[[1]]-new_lambdaVals[[3]])*qFunc(tVals);
  ...
  switch_q = (new_xVals[,1]*new_xVals[,2])*(new_xVals[[1]]-new_xVals[[3]])*pFunc(tVals) - (new_xVals[,3]*new_xVals[,2])*new_lambdaVals[[4]]+(new_xVals[,1]*new_xVals[,2])*(new_lambdaVals[[3]]+new_lambdaVals[[2]]-new_lambdaVals[[1]]);
