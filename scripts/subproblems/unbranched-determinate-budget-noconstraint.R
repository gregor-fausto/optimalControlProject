...
backward = function(t,y,parms,f1,f2, f3, s1, s2,s3,s4) {
  ...
  derivs[1] = beta1t * ( lambda1 * ( 1 - ut) - lambda2 - lambda3 * (1-ut) )
  derivs[2] = 0
  derivs[3] = beta2t * (lambda3 - lambda4 )
  derivs[4] = -(1/L)
...
while(stop_criteria > 1e-5){
  ...
  switch_b1 = (new_lambdaVals[[3]]*(1-uFunc(tVals)) - new_lambdaVals[[1]]*(1-uFunc(tVals)) + new_lambdaVals[[2]] )*P_Func(tVals);
  ...
  switch_b2 = (new_lambdaVals[[4]] - new_lambdaVals[[3]] )*I_Func(tVals);
